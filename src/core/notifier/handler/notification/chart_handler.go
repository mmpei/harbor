...
func (cph *ChartPreprocessHandler) Handle(value interface{}) error {
	...
	if !ok || chartEvent == nil {
		return errors.New("invalid chart event")
	}

	if len(chartEvent.Versions) == 0 || len(chartEvent.ProjectName) == 0 || len(chartEvent.ChartName) == 0 {
		return fmt.Errorf("miss parameters in chart event: %v", chartEvent)
	}
